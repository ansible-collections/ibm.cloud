...
def mock_operations(func):
    def wrapper(self):
        ...
        self.assertIsNone(ibm_resource_bindings_info.MISSING_IMPORT_EXC)

        # Set-up mocks for each operation.
        self.list_patcher = patch('plugins.modules.ibm_resource_bindings_info.ResourceBindingsPager.get_all')
...
class TestResourceBindingsListModuleInfo(ModuleTestCase):
    ...
    def test_list_ibm_resource_bindings_failed(self):
        ...
        self.list_mock.side_effect = ApiException(400, message='List ibm_resource_bindings error')

        with self.assertRaises(AnsibleFailJson) as result:
            os.environ['RESOURCE_CONTROLLER_AUTH_TYPE'] = 'noAuth'
            ibm_resource_bindings_info.main()

        self.assertEqual(result.exception.args[0]['msg'], 'List ibm_resource_bindings error')
