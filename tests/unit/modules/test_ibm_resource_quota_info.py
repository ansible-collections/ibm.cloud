...
def mock_operations(func):
    def wrapper(self):
        ...
        self.assertIsNone(ibm_resource_quota_info.MISSING_IMPORT_EXC)

        # Set-up mocks for each operation.
        self.list_patcher = patch('plugins.modules.ibm_resource_quota_info.ResourceManagerV2.get_quota_definition')
...
class TestQuotaDefinitionModuleInfo(ModuleTestCase):
    ...
    def test_list_ibm_resource_quota_failed(self):
        ...
        self.list_mock.side_effect = ApiException(400, message='List ibm_resource_quota error')
        ...
        self.assertEqual(result.exception.args[0]['msg'], 'List ibm_resource_quota error')
