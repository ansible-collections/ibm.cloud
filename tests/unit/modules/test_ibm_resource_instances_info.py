...
def mock_operations(func):
    def wrapper(self):
        ...
        self.assertIsNone(ibm_resource_instances_info.MISSING_IMPORT_EXC)

        # Set-up mocks for each operation.
        self.list_patcher = patch('plugins.modules.ibm_resource_instances_info.ResourceInstancesPager.get_all')
...
class TestResourceInstancesListModuleInfo(ModuleTestCase):
    ...
    def test_list_ibm_resource_instances_failed(self):
        ...
        self.list_mock.side_effect = ApiException(400, message='List ibm_resource_instances error')

        with self.assertRaises(AnsibleFailJson) as result:
            os.environ['RESOURCE_CONTROLLER_AUTH_TYPE'] = 'noAuth'
            ibm_resource_instances_info.main()

        self.assertEqual(result.exception.args[0]['msg'], 'List ibm_resource_instances error')
